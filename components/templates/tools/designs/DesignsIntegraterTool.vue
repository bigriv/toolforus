...
import { usePen } from "@/composables/tools/designs/paint/pen";
import { useShape } from "@/composables/tools/designs/shape/shape";
...
const mouse = ref({ isDown: false, drag: { start: { x: 0, y: 0 }, end: { x: 0, y: 0 } } });

const ICON_PATH = "/commons/icons/";
const toolList = [
  { value: "cursor", icon: `${ICON_PATH}cursor.svg`, label: "選択" },
  { value: "pen", icon: `${ICON_PATH}pen.svg`, label: "ペン" },
  { value: "shape", icon: `${ICON_PATH}category.svg`, label: "図形" },
  { value: "text", icon: `${ICON_PATH}title.svg`, label: "テキスト" },
  { value: "zoom", icon: `${ICON_PATH}zoom_in.svg`, label: "ズーム" },
];

const {
  canvas, currentTool, canvasRatioList, size, currentCanvasRatio,
  initCanvas, onChangeCanvasRatio, onChangeTool, onChangeCanvasSize,
  selectedObjects, isSelectingSingle, setupEvents,
  save, undo, redo, canUndo, canRedo,
  layers, refreshLayers, syncCanvasOrder, getLayerType, getLayerLabel, getLayerIcon,
  ...
  onSelectionUpdate: (objects) => {
    if (objects.length > 0) {
      reflectFilter();
      if (isSelectingSingle.value) {
        if (isSelectingShape.value) reflectShapeSetting();
        if (isSelectingText.value) reflectTextSetting();
      }
    }
  },
});

const { extensionList, exportFile, onOpenExportModal, onImport, onExport, onDropFile } = useEditorFile(canvas);
const { penSetting } = usePen(canvas);
const {
  shapeList, shapeSetting, currentShape, isShape,
  reflectShapeSetting, generateShape, backupShapeSetting, rollbackShapeSetting,
} = useShape(canvas);
const {
  textSetting, reflectTextSetting, generateTextObject,
  backupTextSetting, rollbackTextSetting, onChangeText,
} = useText(canvas);
const { filter, reflectFilter } = useImageFilter(canvas);
const { cropping, onCropStart, onCropCancel, onCropSubmit } = useCrop(canvas);
const { removeBackground, isProcessing, errorMessage } = useBackgroundRemoval(canvas);
...
const onLayerRenamed = (id: string, newName: string) => {
  ...
  if (o) { renameLayer(o, newName); refreshLayers(); }
};

const onLayerAdded = () => { addLayer("shape"); save(); };
...
<template>
  <div class="c-editor">
    ...
    <WorkspaceLayout
      storage-key="tou.designs-integrater-layout"
      :titles="{
        layer: 'レイヤー',
        canvas: 'キャンバス',
        properties: 'プロパティ',
    ...
    >
      ...
      <template #properties>
        <PropertiesPanel
          ...
          :shape-fill="shapeSetting.fill"
          :shape-border="shapeSetting.border"
          :shape-border-width="shapeSetting.borderWidth"
          ...
          @update:pen-color="penSetting.color = $event"
          @update:pen-size="penSetting.size = $event"
          ...
          @update:shape-fill="shapeSetting.fill = $event"
          @update:shape-border="shapeSetting.border = $event"
          @update:shape-border-width="shapeSetting.borderWidth = $event"
          @shape-color-open="backupShapeSetting"
          @shape-color-cancel="rollbackShapeSetting"
